...
\chapter{O sistema}
	...
	Sobre este componente podem ser exercidos 3 tipos distintos de operação:
	\begin{enumerate}
		\item \textbf{POSTNegotiation}: inserção da informação relativa à iniciação de um novo periodo de negociação;
		\item \textbf{POSTNegotiationOver}: inserção da informação relativa à cessão da negociação em vigor;
