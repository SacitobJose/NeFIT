...
encode_msg_Response(#'Response'{res = F1}, Bin,
		    TrUserData) ->
    if F1 =:= undefined -> Bin;
       true ->
	   case id(F1, TrUserData) of
		 ...
		 begin
		   TrTF1 = id(TF1, TrUserData),
		   e_mfield_Response_importer(TrTF1, <<Bin/binary, 10>>,
					      TrUserData)
		 end;
	     {producer, TF1} ->
		 begin
		   TrTF1 = id(TF1, TrUserData),
		   e_mfield_Response_producer(TrTF1, <<Bin/binary, 18>>,
					      TrUserData)
		 end
	   end
    end.
...
e_mfield_Response_importer(Msg, Bin, TrUserData) ->
    SubBin = encode_msg_ImporterResponse(Msg, <<>>,
					 TrUserData),
    Bin2 = e_varint(byte_size(SubBin), Bin),
    <<Bin2/binary, SubBin/binary>>.

e_mfield_Response_producer(Msg, Bin, TrUserData) ->
    SubBin = encode_msg_DealerTimeout(Msg, <<>>,
				      TrUserData),
    Bin2 = e_varint(byte_size(SubBin), Bin),
    <<Bin2/binary, SubBin/binary>>.
...
dfp_read_field_def_Response(<<10, Rest/binary>>, Z1, Z2,
			    F@_1, TrUserData) ->
    d_field_Response_importer(Rest, Z1, Z2, F@_1,
			      TrUserData);
dfp_read_field_def_Response(<<18, Rest/binary>>, Z1, Z2,
			    F@_1, TrUserData) ->
    d_field_Response_producer(Rest, Z1, Z2, F@_1,
			      TrUserData);
...
dg_read_field_def_Response(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
      10 ->
	  d_field_Response_importer(Rest, 0, 0, F@_1, TrUserData);
      18 ->
	  d_field_Response_producer(Rest, 0, 0, F@_1, TrUserData);
      _ ->
	  case Key band 7 of
...
d_field_Response_importer(<<1:1, X:7, Rest/binary>>, N,
			  Acc, F@_1, TrUserData)
    when N < 57 ->
    d_field_Response_importer(Rest, N + 7, X bsl N + Acc,
			      F@_1, TrUserData);
d_field_Response_importer(<<0:1, X:7, Rest/binary>>, N,
			  Acc, Prev, TrUserData) ->
    {NewFValue, RestF} = begin
			   Len = X bsl N + Acc,
			   <<Bs:Len/binary, Rest2/binary>> = Rest,
			   {id(decode_msg_ImporterResponse(Bs, TrUserData),
			       TrUserData),
			    Rest2}
			 end,
    dfp_read_field_def_Response(RestF, 0, 0,
				case Prev of
				  undefined ->
				      id({importer, NewFValue}, TrUserData);
				  {importer, MVPrev} ->
				      id({importer,
					  merge_msg_ImporterResponse(MVPrev,
								     NewFValue,
								     TrUserData)},
					 TrUserData);
				  _ -> id({importer, NewFValue}, TrUserData)
				end,
				TrUserData).

d_field_Response_producer(<<1:1, X:7, Rest/binary>>, N,
			  Acc, F@_1, TrUserData)
    when N < 57 ->
    d_field_Response_producer(Rest, N + 7, X bsl N + Acc,
			      F@_1, TrUserData);
d_field_Response_producer(<<0:1, X:7, Rest/binary>>, N,
			  Acc, Prev, TrUserData) ->
    {NewFValue, RestF} = begin
			   Len = X bsl N + Acc,
			   <<Bs:Len/binary, Rest2/binary>> = Rest,
			   {id(decode_msg_DealerTimeout(Bs, TrUserData),
			       TrUserData),
			    Rest2}
			 end,
    dfp_read_field_def_Response(RestF, 0, 0,
				case Prev of
				  undefined ->
				      id({producer, NewFValue}, TrUserData);
				  {producer, MVPrev} ->
				      id({producer,
					  merge_msg_DealerTimeout(MVPrev,
								  NewFValue,
								  TrUserData)},
					 TrUserData);
				  _ -> id({producer, NewFValue}, TrUserData)
				end,
				TrUserData).
...
merge_msg_Response(#'Response'{res = PFres},
		   #'Response'{res = NFres}, TrUserData) ->
    #'Response'{res =
		    case {PFres, NFres} of
		      {{importer, OPFres}, {importer, ONFres}} ->
			  {importer,
			   merge_msg_ImporterResponse(OPFres, ONFres,
						      TrUserData)};
		      {{producer, OPFres}, {producer, ONFres}} ->
			  {producer,
			   merge_msg_DealerTimeout(OPFres, ONFres, TrUserData)};
		      {_, undefined} -> PFres;
		      _ -> NFres
		    end}.
...
v_msg_Response(#'Response'{res = F1}, Path,
	       TrUserData) ->
    case F1 of
      undefined -> ok;
      {importer, OF1} ->
	  v_msg_ImporterResponse(OF1, [importer, res | Path],
				 TrUserData);
      {producer, OF1} ->
	  v_msg_DealerTimeout(OF1, [producer, res | Path],
			      TrUserData);
      _ -> mk_type_error(invalid_oneof, F1, [res | Path])
    end,
    ok;
     ...
     {{msg, 'SaleInfo'},
      [#field{name = username, fnum = 1, rnum = 2,
       ...
       #field{name = price, fnum = 3, rnum = 4, type = int64,
	      occurrence = required, opts = []}]},
     {{msg, 'Response'},
      [#gpb_oneof{name = res, rnum = 2,
		  fields =
		      [#field{name = importer, fnum = 1, rnum = 2,
			      type = {msg, 'ImporterResponse'},
			      occurrence = optional, opts = []},
		       #field{name = producer, fnum = 2, rnum = 2,
			      type = {msg, 'DealerTimeout'},
			      occurrence = optional, opts = []}]}]},
...
get_msg_names() ->
    ['Authentication', 'Transaction', 'Produce', 'Import',
     'ServerResponse', 'SaleInfo', 'Response',
...
fqbin_to_msg_name(<<"protos.Response">>) -> 'Response';
...
msg_name_to_fqbin('Response') -> <<"protos.Response">>;
